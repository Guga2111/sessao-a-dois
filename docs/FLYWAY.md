...
- `api/src/main/resources/db/migration/V8__create_password_reset_token.sql` — cria a tabela `password_reset_token` (epico 10, US-004), que ainda nao existe em producao. Guarda so o hash do token de redefinicao de senha (nunca o valor em claro), expiracao, uso e FK para `users.id`, no mesmo molde da `refresh_token` (V5) — ver `docs/SCHEMA_BASELINE.md` — apenas `CREATE TABLE IF NOT EXISTS`/`CREATE INDEX IF NOT EXISTS`, nenhuma tabela existente e tocada.
