  ...
  return (
    ...
    >
      ...
      <div className="relative grid grid-cols-1 gap-[clamp(1.5rem,4vw,2.5rem)] sm:grid-cols-3">
        <div
          aria-hidden="true"
          className="absolute top-[29px] right-[calc(16.6667%+1.25rem)] left-[calc(16.6667%+1.25rem)] hidden border-t-2 border-dashed border-[#ffcb2b]/25 sm:block"
        />
          ...
          return (
            <div key={step.title} className="relative flex flex-col items-center text-center sm:items-start sm:text-left">
              <div className="relative z-10 flex items-center gap-3">
                <div className="grid size-14 flex-none place-items-center rounded-2xl border border-[#ffcb2b]/25 bg-[#161513] shadow-[0_10px_26px_rgba(0,0,0,.35)]">
                  <Icon className="size-6 text-[#ffcb2b]" aria-hidden="true" />
                </div>
                <span
                  aria-hidden="true"
                  className="font-display text-[13px] font-bold text-[#a6a39a]"
                >
                  {`0${index + 1}`}
                </span>
              </div>
              ...
              <p className="font-auth-body mt-2 max-w-[26ch] text-[14px] leading-relaxed text-[#a6a39a]">
                {step.copy}
              </p>
            </div>
          )
