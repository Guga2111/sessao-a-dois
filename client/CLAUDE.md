...
- Hand-rolled `fixed inset-0` backdrop `<div>` — has its own click-outside/Escape-key handling; width/height go directly on the inner div. This group was migrated onto the `Dialog` primitive one file at a time (Epico 13, US-045 through US-052); the modal/dialog files (`DeleteTrackDialog`/`WatchModal`/`ReviewModal`/`MatchCelebrationModal`/`MediaDetailModal`/`PendingDetailModal`/`CoupleSection`'s `DissolveDialog`/`DeleteAccountSection`'s `DeleteDialog`) are now on the primitive — **no hand-rolled backdrop is left in the app** as of US-052. `MatchCelebrationModal.tsx` (US-048) needed a `z-[70]` override on `DialogContent` to preserve its original stacking above the (then still hand-rolled) `screens/account/` dialogs (`z-[60]`) — check whether any newly-converted modal had a non-default `z-` on its old backdrop before dropping it; `CoupleSection.tsx`'s `DissolveDialog` (US-051) had a `z-[60]` on its old backdrop but didn't need an override after conversion, since nothing in the app relies on it stacking above anything else — only drop a hand-rolled `z-` value if you've confirmed (like this one) that no sibling modal depended on the relative order. `MediaDetailModal.tsx` (US-049) and `PendingDetailModal.tsx` (US-050) both split into a thin wrapper (owns `Dialog`/`DialogContent`, static `sr-only` title/description) and a `*ModalContent` component that takes a **non-nullable** `track`/`item` prop, rendered as `{track && <MediaDetailModalContent key={track.id} track={track} .../>}` — same shape as `ReviewModal.tsx`'s `ReviewModalContent` split, needed because both contents' rendering reads the prop's fields pervasively and can't be made null-safe with fallbacks the way `MatchCelebrationModal`'s simpler `matchData?.title` shortcut could. `CoupleSection.tsx`'s `DissolveDialog` (US-051) and `DeleteAccountSection.tsx`'s `DeleteDialog` (US-052) didn't need that split — their content only reads plain props (`partnerName`/`bondLabel`, or `partnerName`/`onClose` for the delete dialog), never a nullable object with pervasive field access. The two do differ from each other in mounting shape, though: `DissolveDialog` kept its pre-existing "always mounted from the parent, `open={confirming}` boolean" pattern (same as `MatchCelebrationModal`); `DeleteDialog` kept its pre-existing **conditional** parent-mount (`{confirming ? <DeleteDialog .../> : null}`, `Dialog` given a hardcoded `open`) instead of switching to the always-mounted shape, specifically because its form has local state (`password`/`word`/`working`/`done`) that needs to reset on every reopen — unmounting is what gives that reset for free, and `DissolveDialog`'s simpler `working`/`error` state was never guaranteed to reset on reopen either (an existing, accepted quirk, not something US-052 needed to fix or replicate).
...
## Badge / Chip (Epico 13, US-054, migration started US-055)

`src/components/ui/badge.tsx` (`Badge`, static `<span>`, `tone: neutral | primary | coral | muted`) and `src/components/ui/chip.tsx` (`Chip`, interactive `<button>`, `active: true | false`, native `disabled`) are the two primitives for the "pill" shape (`rounded-full` + `border` + small padded text) that used to be hand-rolled per call site — a real design decision, not a governance/refactor task, so it went through the `frontend-design` skill (E13.4) rather than the pure-refactor exemption. **Two primitives, not one**: `Badge` is a display-only span (no `disabled`/`cursor-pointer`/`aria-pressed`), `Chip` is an actual button with interaction state — merging them into one component with an "interactive" variant would mix a display API with an action API, same reason `Button` and `ToggleGroupItem` stay separate despite sharing part of their shell. Both export their `cva` function (`badgeVariants`/`chipVariants`) alongside the component, same pattern as `button.tsx`'s `buttonVariants` (already consumed by `pagination.tsx`) — reuse that export, don't hand-copy the className string, when a non-`Badge`/`Chip` element needs to look like one (e.g. `FiltersPanel.tsx`'s sort-`SelectTrigger`, hand-styled to visually match the filter `Chip` beside it — the decided fix is `className={chipVariants({ active: false })}` on that `SelectTrigger`, not a new prop on `select.tsx` itself, which stays a generic, use-case-agnostic primitive).

**Migrating a call site (US-055+): `tone`/base classes are a starting point, not a mandate to change pixels.** `badgeVariants`'s base padding/font-size (`px-2.5 py-1 text-[11px]`) and each `tone`'s exact border/bg/text opacity are defaults for *new* usage — an existing hand-rolled pill being migrated almost always has slightly different padding/opacity/radius than the closest tone, and the story's AC is pixel-equivalence, not tone-purity. Pick the closest `tone` for the color *family*, then override the differing utility classes via `className` (`cn()`/`twMerge` resolves same-category conflicts like `border-primary/30` vs `border-primary/25` in favor of the later `className` value) — don't round the pill's actual colors/spacing to whatever the tone's defaults happen to be. Three patterns seen migrating `MediaCard.tsx`/`PendingDetailModal.tsx`/`MediaDetailModal.tsx` (US-055): (1) exact tone match, only size differs (genre/provider chips → `tone="neutral"` + `className="px-3 py-0.5 text-[12px]"`, or `"flex px-3 py-1 text-[12px]"` for the provider chip that also needs `flex` instead of the base's `inline-flex flex-none` since it wraps an icon+text pair); (2) same color family, different opacity/radius (`MediaCard`'s media-type badge, `PendingDetailModal`'s "Sugestao" badge → `tone="primary"` + `className` overriding `rounded-md`/exact `border-primary/X`/`bg-primary/X`); (3) fully runtime-computed colors that don't map to any static tone at all (`MediaDetailModal`'s per-status badge, colors picked from a `STATUS_BADGE` record keyed by track status) → `tone="neutral"` as a shell placeholder + the same `style={{ background, border, color }}` object as before, passed straight through to `Badge` (it spreads `...props` onto the `<span>`, and inline `style` always wins over the tone's classes) — don't try to force three arbitrary runtime colors into the 4-tone enum. Re-measure real pill usage via `grep -rn "rounded-full\|rounded-md"` per file before assuming which spans are actual badges — a hit next to `border`+`bg`+padding+text-size is a pill; a hit on an avatar/status-dot/icon-button/already-`Button`-primitive element (`size-2 rounded-full` status dots, `size-6` circular avatars, `<Button>` with `rounded-full` in its own `className`) is not, and US-054's own audit undercounted the PRD's original estimate for exactly this reason — don't count Button-primitive elements as "manual pills" even if they visually look pill-shaped, they're already on a primitive.

**Migrating an interactive trigger to `Chip` (US-056):** a base-ui `*Trigger` component (`CollapsibleTrigger`, `TooltipTrigger`, `PopoverTrigger`, etc.) can take a `render={<Chip active={...} />}` prop, same mechanism `TooltipTrigger render={<Button .../>}` already uses elsewhere — the trigger clones its own open/close/`aria-*` wiring onto the rendered element while children stay on the `*Trigger` itself. Use this (not `chipVariants` on a `className`) whenever the element that needs to look like a `Chip` *is* the interactive trigger; reserve `className={chipVariants({ active })}` for a element that's driven by something else (e.g. `Select`'s own open state) and just needs to visually match a `Chip` beside it, like `FiltersPanel.tsx`'s sort `SelectTrigger`. When migrating a toggle/active pill (as opposed to a static `Badge`), diff each state's hover rule individually against `chipVariants`'s output — its `active: true` branch defines no hover, so a call site with a distinct active-hover (`compareUi.tsx`'s `CompareToggleButton`, `hover:bg-primary/18 hover:text-primary`) needs it added back via `className`, not dropped.

Migration stories: `prd.json`'s US-055 (done: `PendingDetailModal.tsx`/`MediaDetailModal.tsx`/`MediaCard.tsx`) and US-056 (done: `FiltersPanel.tsx`'s Filtros-chip + sort-`SelectTrigger`, `compareUi.tsx`'s `CompareToggleButton`; `SuggestionsTab.tsx`/`SearchTab.tsx` had zero real manual pills once re-measured) through US-058 cover the remaining files (`re-measure via grep before splitting further work — the PRD's original "36 usages/18 files" count is stale, US-054's own audit found 15 real badge/chip usages, and not every file named in a "part N/4" split actually has migration work`).
