...
- **CI gate against raw color (Epico 13, US-002/D18):** `scripts/check-color-literals.mjs` (`bun run check:colors`, wired into `.github/workflows/ci.yml`'s `frontend` job) fails the build if a hex literal (`#rrggbb`) or arbitrary Tailwind color utility (`bg-[...]`/`text-[...]`/`border-[...]`/`ring-[...]`/`from-[...]`/`to-[...]`/`via-[...]`/`shadow-[...]`/`fill-[...]`/`stroke-[...]`) shows up outside `src/components/ui/` (permanently exempt — it's the design-system primitives) in a file **not** listed in `color-migration-allowlist.txt`. The allowlist is the migration scoreboard, not a permanent exception: when a story migrates a file to tokens (US-003+), remove that file's line from the allowlist **only once `node scripts/check-color-literals.mjs --list-offenders` (from `client/`) no longer lists it** — i.e. only when it has genuinely zero raw color left, not merely "migrated as much as we could." Regenerate the allowlist (never hand-edit the file list) with that same `--list-offenders` command. Test files (`*.test.tsx`/`*.spec.tsx`, anything under a `test/` directory) are exempt everywhere, not just inside the allowlist.
- **Per-file migration stories routinely can't zero out a file (learned in US-003 on `ComparisonDialog.tsx`):** US-001 only tokenized the 8 *most frequent* hexes app-wide, not every color any single file happens to use. A large file easily has 5-10 extra one-off colors (secondary accents, shadow colors, translucent whites/blacks at an alpha the `--border` token doesn't match, etc.) with no corresponding token. When that happens: migrate whatever maps, leave the rest exactly as it was (never invent a new arbitrary value or a new token in a migration story — that's US-001's job), and **keep the file in `color-migration-allowlist.txt`** rather than breaking the `check:colors` CI gate over a handful of genuinely tokenless leftovers. Document the specific leftover values in the story's `notes` field so a future story can decide whether to extend US-001's token set for them.
- **`rgba(R,G,B,X)` arbitrary utilities that share a token's RGB (confirmed US-003/US-004):** `bg-[rgba(R,G,B,X)]`/`border-[rgba(R,G,B,X)]` whose R,G,B exactly equals a known token's hex (e.g. `rgba(255,203,43,X)` = `#ffcb2b` = `--primary`, `rgba(255,107,107,X)` = `#ff6b6b` = `--destructive`) converts to `token-name/X` for *any* alpha `X` via Tailwind's opacity modifier — no exact-alpha match needed, unlike reusing a token for a *different* property (`rgba(255,255,255,.1)` → `border-border` is only valid because `.1` is `--border`'s exact value; `bg-[rgba(255,255,255,.05)]` has no `bg-` equivalent and must stay arbitrary). A token hex appearing outside a `className` (a plain object field, or `style={{ background: "#hex" }}`) still counts and should become `"var(--token-name)"`, same prop/structure, not left alone just because it's not a Tailwind class.
