...
Migration stories: `prd.json`'s US-055 (done: `PendingDetailModal.tsx`/`MediaDetailModal.tsx`/`MediaCard.tsx`) through US-058 cover the remaining files (`re-measure via grep before splitting further work — the PRD's original "36 usages/18 files" count is stale, US-054's own audit found 15 real badge/chip usages`).
