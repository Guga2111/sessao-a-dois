...
class MediaTrackMapperTest {
	...
	void toResponseUsesResolvedNamesForBothMembersReviews() {
		...
		MediaTrack track = new MediaTrack(UUID.randomUUID(), 603L, MediaType.MOVIE, MediaStatus.WATCHED);
		...
		MediaTrackResponse response = mapper.toResponse(track, List.of(user1Id, user2Id), userNames);
	...
	void toResponseOmitsReviewForMemberWithoutOne() {
		...
		MediaTrackResponse response = mapper.toResponse(track, List.of(user1Id), Map.of(user1Id, "Ana"));
	...
	void toResponseFallsBackToNullNameWhenNotResolved() {
		...
		MediaTrackResponse response = mapper.toResponse(track, List.of(user1Id), Map.of());
